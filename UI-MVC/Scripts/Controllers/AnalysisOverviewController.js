angular.module('sussol.controllers')
    ...
    function ($scope, $sce, $window, $compile, $http, $routeParams, algorithmNames, constants, result, $timeout, organisation, $rootScope) {
        var solvents = [];
        ...
        function showClusterAnalysis(modelsTemp) {
            ...
            for (var teller = 0; teller < models.length; teller++) {
                ...
                for (var i = 0; i < models[teller].Model.Clusters.length; i++) {
                    ...
                    var deviateForCNS = {};
                    deviatePerCluster.push(getStandardDev(solventenincluster));
                    deviateForCNS.model = models[teller].Id;
                    deviateForCNS.cluster = i;
                    deviateForCNS.standarddeviate = getStandardDev(solventenincluster);
                    standarddeviate.push(deviateForCNS);
                }
        ...
        function setEnumNames() {
            ...
            for (var i = 0; i < models.length; i++) {
                models[i].Model.AlgorithmName = algorithmNames.data[models[i].Model.AlgorithmName];
        ...
        function createJsonModel(model) {
            ...
            for (var i = 0; i < model.Model.Clusters.length; i++) {
                ...
                var selecteddev;
                for (var j = 0; j < standarddeviate.length; j++) {
                    if (standarddeviate[j].model === model.Id && standarddeviate[j].cluster == i) {
                        selecteddev = standarddeviate[j].standarddeviate;
                    }
                }
                json[i] = ({
        ...
        $scope.showSolventInfo = function (casnumber) {
            ...
            hidedetails();
            var instance
            if (prevClassifiedInstances != undefined && prevClassifiedInstances.length > 0){
